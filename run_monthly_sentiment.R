#!/usr/bin/env Rscript
# ---------------------------------------------------------------------------
# run_monthly_sentiment.R  (numeric)
# ---------------------------------------------------------------------------
# * Renders monthly_tweet_report.Rmd  → HTML
# * Prints the HTML to PDF (pagedown + headless Chrome)
# * Uploads the PDF to Supabase (bucket: monthly-numeric/YYYYMM/…)
...
month_start  <- suppressWarnings(as.Date(m_env)) %||% lubridate::floor_date(Sys.Date() - 1, unit = "month")
  ...
  mj_resp <- request("https://api.mailjet.com/v3.1/send") |>
    ...
    req_body_json(list(
      Messages = list(list(
        ...
        Subject     = sprintf("Monthly Numeric Report – %s", format(month_start, "%B %Y")),
